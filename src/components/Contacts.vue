<template>
  <div class="testimonials-page">
    ...
    <div class="testimonials-section">
      ...
      <!-- Looping through messages to display each one -->
      <div class="testimonial" v-for="testimonial in testimonials" :key="testimonial.id">
        <blockquote>{{ testimonial.quote }}</blockquote> <!-- Displaying message -->
        <cite>- {{ testimonial.author }}</cite> <!-- Displaying author of the message -->
      </div>

      <!-- Comment Form for submitting new messages -->
      <div class="comment-form">
        <h4>Leave a Message</h4> <!-- Changed the header to "Leave a Message" -->
        <textarea v-model="newComment" placeholder="Your message"></textarea> <!-- Textarea for user to enter message -->
        <input v-model="newAuthor" type="text" placeholder="Your Name" /> <!-- Input for user's name -->
        <button @click="submitComment">Submit</button> <!-- Button to submit message -->
      </div>
...
export default {
  ...
  data() {
    return {
      // Initial messages array with sample data
      testimonials: [
        {
          id: 1,
      ...
      newComment: '', // Holds the new message input
  ...
  methods: {
    ...
    submitComment() {
      ...
      if (this.newComment && this.newAuthor) {
        ...
        this.testimonials.push(newTestimonial); // Add new message to the array
